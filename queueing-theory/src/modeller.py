...
class PoissonDistribution:
    ...
    def next(self):
        return poisson.rvs(self._lmbda, size=1)[0]
...
class RequestProcessor(RequestGenerator):
    ...
    def process(self):
        if self._current_queue_size > 0:
            ...
            self.emit_request()
            if nr.random_sample() < self._reenter_probability and self._reentered_requests < self._reenter_probability * PRB:
...
class Modeller:
    def __init__(self, uniform_a, uniform_b, expo_lambda, reenter_prop):
        ...
        self._processor = RequestProcessor(PoissonDistribution(expo_lambda), reenter_prop)
